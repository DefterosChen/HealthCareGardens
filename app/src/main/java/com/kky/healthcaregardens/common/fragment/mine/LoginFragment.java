...
import com.kky.healthcaregardens.common.base.utils.EncryptAES;
import com.kky.healthcaregardens.common.fragment.MainFragment;
import com.kky.healthcaregardens.common.fragment.WebPageFragment;
...
import com.kky.healthcaregardens.helper.lunzi.InputTextHelper;
...
//        }
    ...
    private void loginByPassword() {
        ...
        String passwordAES = EncryptAES.Encrypt(mPasswordView.getText().toString(), ServiceCall.AES_KEY);


        System.out.println("password:" + mPasswordView.getText().toString() + "---------------passwordAES:" + passwordAES);

        String url = ServiceCall.getServerURL() + "/auth/oauth/token?username=" + mPhoneView.getText().toString()
                + "&password=" + URLEncoder.encode(passwordAES)
                + "&randomStr=" + timeStr
                + "&code=" + mPicCodeView.getText().toString()
     ...
     * @param returnBody
     */
    private void getUserInfo(String returnBody) {
        //转换返回数据格式
        Type type = new TypeToken<LoginUser>() {
        }.getType();
        LoginUser loginUser = new Gson().fromJson(returnBody, type);
        System.out.println("返回的数据：" + loginUser.toString());
        loginUser.setMobile(mPhoneView.getText().toString());

        //保存用户登录获取的数据
        BaseUtils.saveLogin(getActivity(), new Gson().toJson(loginUser));
        //跳转到首页
    ...
    private void getPicCode() {
        ...
        timeStr = BaseUtils.getRandom(4) + DateTimeUtil.getCurrentTimestamp().getTime() / 1000;  // 随机四位数 + 当前时间戳
        System.out.println(" timeStr = " + timeStr);

        String url = ServiceCall.getServerURL() + "/code?randomStr=" + timeStr;
